...
const { assertPercent, fromPercent, isPercent } = nameHelpers(percent, "Percent");
const { assertIPv4Address, isIPv4Address, fromIPv4Address } = nameHelpers(ipv4Address, "IPv4Address");
...
let p1: Percent = percent.from(1);
let p2: PositiveInteger = positiveInteger.from(1);
let p3: number = 1;
...
// A way to bypass that behaviour would be to use to 'to' function, but it requires to store the result in a new strictly typed variable
let _1 = ipv4Address.from(s1); // s1 is 'IPv4Address'
...
let s3: string = "";
let s4 = ipv4Address.from(s3); // s4 is 'IPv4Address'
